...
async function autoMineZeroDelayUltraPro_v4() {
  ...
  // 🔁 Vòng lặp chính
  async function loop() {
    if (STOP_AUTOMINE) return showMessage("🛑 Dừng khẩn cấp!");
  ...
  loop();
    ...
    document.addEventListener("keydown", e => {
        ...
        if (e.code === "Space") {STOP_AUTO = true; STOP_AUTOMINE = true};// Dừng khẩn cấp
    });
})();
